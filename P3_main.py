...
def trainDualStreamModel_main(opt):
    opt = extend_opt(opt)
    ...
    fnames = {'train': get_csv_fname('train', opt.dataset, opt.datasetNum),
              'eval': get_csv_fname('eval', opt.dataset, opt.datasetNum),
              'test': get_csv_fname('test', opt.dataset, opt.datasetNum)}

    datasets = {'train': read_csv(fnames['train'], opt.min_seq_len),
                'eval': read_csv(fnames['eval'], opt.min_seq_len),
                'test': read_csv(fnames['test'], opt.min_seq_len)}
    ...
    model = DKTInSkillQues(nSkill=opt.n_skill,
                           nQues=opt.n_question,
                           szRnnIn=opt.sz_rnn_in,
                           szRnnOut=opt.sz_rnn_out,
                           nRnnLayer=opt.n_rnn_layer,
                           szOut=opt.n_skillPad,
