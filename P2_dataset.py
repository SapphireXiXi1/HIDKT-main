...
class KTDataset(Dataset):
    def __init__(self, seqLen_allSample, questionID_allSample, skillID_allSample, label_allSample, n_question, n_skill, opt):
        ...
        self.eyeSkill = np.eye(n_skill + 1, dtype=np.float32)
        self.opt = opt
    ...
    def __getitem__(self, index: int):
        ...
        questionID_1Sample = np.asarray(self.questionID_allSample[index], dtype=np.int32)
        skillID_1Sample = np.asarray(self.skillID_allSample[index], dtype=np.int32)
        label_1Sample = np.asarray(self.correctness_all[index], dtype=np.int32)

        currQuestionAddLabel_1Sample = 2 * questionID_1Sample[:-1] + label_1Sample[:-1]
        currQuestionID_1Sample = questionID_1Sample[:-1]
        currSkillAddLabel_1Sample = 2 * skillID_1Sample[:-1] + label_1Sample[:-1]
        ...
        return torch.tensor([effLen_1Sample], dtype=torch.long), \
               torch.from_numpy(currQuestionAddLabel_1Sample.astype(np.int64, copy=False)), \
               torch.from_numpy(currQuestionID_1Sample.astype(np.int64, copy=False)), \
               torch.from_numpy(currSkillAddLabel_1Sample.astype(np.int64, copy=False)), \
               torch.from_numpy(currSkillID_1Sample.astype(np.int64, copy=False)), \
               torch.from_numpy(currSkill_oneHot_1Sample), \
               torch.from_numpy(currLabel_1Sample.astype(np.float32, copy=False)), \
               torch.from_numpy(nextQuestionID_1Sample.astype(np.int64, copy=False)), \
               torch.from_numpy(nextSkillID_1Sample.astype(np.int64, copy=False)), \
               torch.from_numpy(nextSkill_oneHot_1Sample), \
               torch.from_numpy(nextLabel_1Sample.astype(np.float32, copy=False))
